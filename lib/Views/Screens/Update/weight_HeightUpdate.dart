...
class _Weight_HeightUpdate extends State<Weight_HeightUpdate> {
    ...
    return Scaffold(
      ...
      body: Column(
        ...
        children: [
          ...
          ElevatedButton(
            onPressed: () {
              BodyMetricsSingleton singleton =
                  BodyMetricsSingleton.getInstance();
              BodyMetrics metrics = singleton.getMetrices();
  ...
  Widget _buildImperialWeightPicker(BuildContext context) {
    return update_widget.textFormFieldWidget("Select Weight", selectedWeight,
        context, "Pounds", _buildNumberPickerInPound);
  }

  Widget _buildMetricWeightPicker(BuildContext context) {
    return update_widget.textFormFieldWidget("Select Weight", selectedWeight,
        context, "Kilograms", _buildNumberPickerInKg);
  }

  Widget _buildMetricPicker(BuildContext context) {
    return update_widget.textFormFieldWidget("Select Height ", selectedHeight,
        context, "Centimeter", _buildNumberPickerInCm);
  }

  Widget _buildImperialPicker(BuildContext context) {
    return update_widget.textFormFieldWidget("Select Height ", selectedHeight,
        context, "Feet & Inches", _buildTwoNumberPicker);
  }
  ...
  Widget _buildNumberPicker(
      int x, String text, int minValue, int maxValue, double itemWidth) {
    return Row(
      mainAxisAlignment: MainAxisAlignment.center,
      children: [
        StatefulBuilder(
            ...
            return NumberPicker(
              ...
              decoration: const BoxDecoration(
                border: Border(
                  ...
                  bottom: BorderSide(
                      color: Colors.white, width: 3), // Color for bottom border
                ),
              ),
            );
          },
