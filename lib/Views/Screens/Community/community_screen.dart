...
class _communityPage extends State<CommunityPage> {
  late Future<List<dynamic>>
      activities; // Declare _activitiesFuture as a Future<List<dynamic>>
  ...
  Future<void> fetchAndSetActivities() async {
    activities =
        fetchActivities(); // Assign the result of fetchActivities() to _activitiesFuture
  }
  ...
  Future<List<dynamic>> fetchActivities() async {
    ...
    List<dynamic> allActivities =
        await controller.getRecentActivities(user.id); // Get all activities
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          Expanded(child: displayLatestActivities()),
        ],
      ),
    );
  }
  ...
  Future<Widget> fetchReview(Review activity) async {
    ...
    return communityReviewsWidget.CommunityreviewsWidget(
        workout!, activity, user!, formatTimestamp(activity.timestamp));
  }
  ...
  Future<Widget> fetchWorkout(Workout activity) async {
    ...
    return communityWorkoutWidget.communityCardWidget(
        ...
        context,
        user!.name,
        CommunityPage.likedWorkoutIds.contains(activity.id), (isLiked) {
      setState(() {
        if (isLiked) {
